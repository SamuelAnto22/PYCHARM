...
class BibliotecaComicEjemplar(models.Model):
    ...
    codigo_ejemplar = fields.Char(string="Código de Ejemplar", required=True)

    socio_id = fields.Many2one('biblioteca.socio', string="Prestado a")
    fecha_inicio_prestamo = fields.Date(string="Fecha de Préstamo")
    fecha_prevista_devolucion = fields.Date(string="Fecha Prevista Devolución")

    @api.constrains('fecha_inicio_prestamo')
    def _check_fecha_prestamo(self):
        hoy = fields.Date.today()
        for record in self:
            if record.fecha_inicio_prestamo and record.fecha_inicio_prestamo > hoy:
                raise ValidationError("La fecha de préstamo no puede ser posterior al día de hoy.")

    @api.constrains('fecha_prevista_devolucion')
    def _check_fecha_devolucion(self):
        hoy = fields.Date.today()
        for record in self:
            if record.fecha_prevista_devolucion and record.fecha_prevista_devolucion < hoy:
